...
BYTE *GetRowStart(_In_ CONST BLT_INFO *pBltInfo, _In_ CONST RECT *pRect)
{
    ...
    {
        case D3DKMDT_VPPR_IDENTITY:
            pRet = ((BYTE *)pBltInfo->pBits + (ULONGLONG)OffTop * pBltInfo->Pitch + (ULONGLONG)OffLeft * BytesPerPixel);
            break;
        case D3DKMDT_VPPR_ROTATE90:
            pRet = ((BYTE *)pBltInfo->pBits + ((ULONGLONG)pBltInfo->Height - 1 - OffLeft) * pBltInfo->Pitch +
                    (ULONGLONG)OffTop * BytesPerPixel);
            break;
        case D3DKMDT_VPPR_ROTATE180:
            pRet = ((BYTE *)pBltInfo->pBits + ((ULONGLONG)pBltInfo->Height - 1 - OffTop) * pBltInfo->Pitch +
                    ((ULONGLONG)pBltInfo->Width - 1 - OffLeft) * BytesPerPixel);
            break;
        case D3DKMDT_VPPR_ROTATE270:
            pRet = ((PBYTE)pBltInfo->pBits + (ULONGLONG)OffLeft * pBltInfo->Pitch +
...
VOID CopyBitsGeneric(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc, _In_ CONST RECT *pRect)
{
    ...
    for (UINT y = 0; y < NumRows; y++)
    {
        ...
        for (UINT x = 0; x < NumPixels; x++)
        {
            ...
            else if ((pDst->BitsPerPel == 24) || (pSrc->BitsPerPel == 24))
            {
...
VOID CopyBits32_32(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc, _In_ CONST RECT *pRect)
{
    VIOGPU_ASSERT((pDst->BitsPerPel == 32) && (pSrc->BitsPerPel == 32));
    VIOGPU_ASSERT((pDst->Rotation == D3DKMDT_VPPR_IDENTITY) && (pSrc->Rotation == D3DKMDT_VPPR_IDENTITY));
    ...
    BYTE *pStartDst = ((BYTE *)pDst->pBits + ((ULONGLONG)pRect->top + pDst->Offset.y) * pDst->Pitch +
                       ((ULONGLONG)pRect->left + pDst->Offset.x) * 4);
    CONST BYTE *pStartSrc = ((BYTE *)pSrc->pBits + ((ULONGLONG)pRect->top + pSrc->Offset.y) * pSrc->Pitch +
...
VOID BltBits(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc, _In_ CONST RECT *pRect)
{
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
    __try
    {
        if ((pDst->Rotation == D3DKMDT_VPPR_IDENTITY && pSrc->Rotation == D3DKMDT_VPPR_IDENTITY) &&
            (pDst->BitsPerPel == 32 && pSrc->BitsPerPel == 32))
        {
